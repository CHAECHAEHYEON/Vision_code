...
Mat StereoVision::find_edge(Mat& frame, int camera) {
	...
	// GaussianBlur(gray_image,gray_image, Size(5,5),4);
	// imshow("gau", gray_image);

	Mat bilater_image;
	bilateralFilter(gray_image, bilater_image, 3,10,10);
	// imshow("filter", bilater_image);
	
	cout << " ! " << endl;
	Mat dx, dy;
	Sobel(bilater_image, dx, CV_32FC1, 1, 0);
	Sobel(bilater_image, dy, CV_32FC1, 0, 1);
	...
	for (int r = 0; r < frame.rows; r++)
	{
		...
		{
			if((5*bilater_image.at<uchar>(r, c) - bilater_image.at<uchar>(r, c+1) - bilater_image.at<uchar>(r, c+2) - bilater_image.at<uchar>(r, c+3) - bilater_image.at<uchar>(r, c+4) - bilater_image.at<uchar>(r, c+5)) > 80) // 60
			{
	...
	for (int r = 0; r < frame.rows; r++)
	{
		...
		{
			if((bilater_image.at<uchar>(r, c+1) + bilater_image.at<uchar>(r, c+2) + bilater_image.at<uchar>(r, c+3) + bilater_image.at<uchar>(r, c+4) + bilater_image.at<uchar>(r, c+5)) - 5*bilater_image.at<uchar>(r, c) > 80) // 60
			{
				dst.at<uchar>(r, c-2) = 255;
				dst.at<uchar>(r, c-1) = 255;
				dst.at<uchar>(r, c) = 255;
	...
	circle(bilater_image, Point(center_x,center_y), 10, Scalar(255, 255, 255), 2);

	if(camera == 0)
	{
		// imshow("left circle mean", bilater_image);
	}
	else
	{
		// imshow("right circle mean", bilater_image);
	}
	subtract(edge, dst, edge);
	imshow("Minus", edge);

	dilate(edge, edge, Mat(), Point(-1, -1), 4);
