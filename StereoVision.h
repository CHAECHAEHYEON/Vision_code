...
public:
	...
	Point mass_center_left(Mat& img, int count);
	Point mass_center_right(Mat& img, int count);

	void line_symmetry(Mat& frame, int camera);

	float* mass_center(Mat &img, float array[], int camera);
	Point2f find_XZ(Point2f circleLeft, Point2f circleRight, Mat& leftFrame, Mat& rightFrame, float alpha, float beta);
