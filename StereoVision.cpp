...
Mat StereoVision::add_HSV_filter(Mat& frame, int camera) {
	...
	vector<int> lowerLimitRedRight = { 0, 0, 180 };     // Lower limit for white
	vector<int> upperLimitRedRight = { 255, 100, 255 };	 // Upper limit for white
	vector<int> lowerLimitRedLeft = { 0, 0, 180 };     // Lower limit for white
...
Point StereoVision::mass_center_left(Mat& img, int count)
{
    Mat mask2 = img.clone();
    ...
    if(contours.size() > 0)
    {
        ...
        circle(drawing, Point(mean_bottom_x, mean_bottom_y), 10, Scalar(255, 0, 0), -1);
        ...
        {
            return {mean_bottom_x, mean_bottom_y};
        }
    }
    else
    {
...
Point StereoVision::mass_center_right(Mat& img, int count)
{
	Mat mask2 = img.clone();
...
float *StereoVision::mass_center(Mat &img, float array[], int camera)
{
    ...
    if(contours.size() > 0)
    {
        ...
        circle(drawing, Point((int)mean_top_x, (int)mean_top_y), 10, Scalar(0, 255, 0), -1);
...
Point2f StereoVision::find_XZ(Point2f left_point, Point2f right_point, Mat& leftFrame, Mat& rightFrame, float alpha, float beta)
{	
	float x_0 = 0;
	float y_0 = 0;
