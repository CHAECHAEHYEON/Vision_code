    ...
    try
    {
		...
		for(int i = 0; i < detect_length; i++)
		{
            ...
            else if (Class_name == "B1" || Class_name == "B2" || Class_name == "B3")
            {
                ...
                if ((Area>2300) &&( (boundingBox2d.size_y)/(boundingBox2d.size_x)<=1.5) && (mission_flag==8))   //2700
                {
			...
			else if ((Class_name == "A3") && (Area >2300) && ( (boundingBox2d.size_y)/(boundingBox2d.size_x)<=1.5) && (mission_flag==8))
			{
...
int main(int argc, char **argv)
{
	...
	detect_length_sub = nh.subscribe("/yolov7/detect_length", 100, detectionlength_callback);
	...
	// Korus ver. topic
    traffic_sign_pub = nh.advertise<std_msgs::Int32MultiArray>("Vision/traffic_sign_1", 100);
	...
	VideoCapture cap("/home/korus/catkin_ws/src/VISION/src/video/0923_kcity/kcity_0923_04.mp4");
