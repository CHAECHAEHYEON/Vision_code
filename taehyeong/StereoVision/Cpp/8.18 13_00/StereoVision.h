...
public:
	...
	StereoVision(float baseline, float focal_pixels, float alpha, float beta)
	: baseline(baseline),focal_pixels(focal_pixels), alpha(alpha), beta(beta){}
	...
	float* mass_center(Mat &img, float array[], int camera);
	Point2f find_XZ(Point2f circleLeft, Point2f circleRight, Mat& leftFrame, Mat& rightFrame);
