...
float alpha = 23;	//alpha = 카메라 머리 숙인 각도
float beta = 42; 	//beta = erp 헤딩으로부터 카메라 각도
int encoder = 3;
bool enc_fail = false;
...
{   
    ...
    VideoCapture capLeft(6); //9
    VideoCapture capRight(4); //1
    ...
    Parking_level3_sub = nh.subscribe("/Parking/cam", 10, parking_cb_3); //on publish stopline distance
	Parking_encoder_fail_sub = nh.subscribe("enc_fail", 10, enc_fail_cb);
	encoder_pub = nh.advertise<std_msgs::Int32>("encoder_mode", 100);
    ...
    while (ros::ok()) 
    {
		...
		std_msgs::Int32 encoder_mode_msg;
        ...
        else
		{
            ...
            else if((parking_level_1 == true) && (parking_level_2 == false) && (parking_level_3 == false)) //(parking_level_2 == false) &&  (parking_level_3 == false)
            {	
				encoder = 2;
				imshow("Left_mask", leftMask);
           		imshow("Right_mask", rightMask);
                ...
                float *ptr_left = stereovision.find_center(leftMask, left_array, 0);
                float *ptr_right = stereovision.find_center(rightMask, right_array, 1);
				...
				for(int i=0; i<6; i++)
				{
					center_XZ_msg.data.push_back(array[i]);
					// printf("array[] : %f\n", array[i]);
				}
				center_XZ_pub.publish(center_XZ_msg);
            }
			else if((parking_level_1 == true) && (parking_level_2 == true) && (parking_level_3 == false) && FinalLevel == false)
			{
				encoder = 2;
				cout << " waiting for signal " << endl;
			}
            else if((parking_level_1 == true) && (parking_level_2 == true) && (parking_level_3 == true)) //else if
            {
				encoder = 2;
...
Mat find_stopline(Mat &img)
{
	...
	std_msgs::Float64 stopline_msg2;
	stopline_msg.data = count;
	stopline_msg2.data = 10;
	// cout << "stopline" << endl;
	...
	{
		stopline_pub.publish(stopline_msg2);
	}
