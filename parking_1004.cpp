...
{   
	...
	VideoCapture capLeft("/home/korus/catkin_ws/src/record_video/stereo_video/right_0917_2.mp4");
    VideoCapture capRight("/home/korus/catkin_ws/src/record_video/stereo_video/left_0917_2.mp4");
    ...
    while (ros::ok()) 
    {
		...
		if((mission_flag == 10)||(mission_flag == 6))
		{
			...
			{	
				...
				double default_array[6] = {2.4, -2.2, 3.65, -4.2, 4.9, -6.2};
//#############################################3좌표계산######################################################################################
	...
	//############################################ 보험용 #################################################################################################
					...
					if(array_count == 20)
					{
						...
						// if((abs(pub_array[2]-default_array[2]) > 1.5) || (abs(pub_array[3]-default_array[3]) > 1.5))
						// {
						// 	cout << "주차 실패 ㅠㅠ" << endl;
						// 	center_XZ_msg.data.clear();
						// 	for(int i=0; i < 6; i++)
						// 	{
						// 		pub_array[i] = default_array[i];
						// 		center_XZ_msg.data.push_back(pub_array[i]);
						// 	}
						// }
