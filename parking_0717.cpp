...
Point parking_level1(Mat frame, int delay)
{
    ...
    {
        if(sqrt(pow(hull[i].x - 640, 2) + pow(hull[i].y - 0, 2))<= sqrt(pow(max_x - 640, 2) + pow(max_xy - 0, 2)))
        {
            max_x = hull[i].x;
			max_xy = hull[i].y;
            max_x_num = i;
...
int main(int argc, char **argv)
{
    ...
    // VideoCapture cap("/home/kroad/catkin_ws/src/parking/src/parking_video/kcity1_parking2.mp4");
	VideoCapture cap(6);
    ...
    while(ros::ok())
    {
        ...
        else if(parking1 == true && third_level == false)
        {
            ...
            inRange(src_HLS, Scalar(0, 170, 0), Scalar(255, 255, 255), src_HLS);
...
Mat bird_eyes_view_for_front_camara(Mat img)
{
	...
	// warp_src_point[0].x = 15; aaaaaaaaaa
